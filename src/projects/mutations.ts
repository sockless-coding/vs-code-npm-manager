...
import { DependencyType, MutationRequest, MutationResult, VersionPrefix } from "../panel/messaging";
...
import { applyVersionPrefix, detectVersionPrefix, toCaretRange, toExactVersionPin } from "../npm/versionRange";
...
export class MutationService {
  ...
  async apply(req: MutationRequest, registryUrl?: string): Promise<MutationResult> {
    ...
    if (result.ok && vscode.workspace.getConfiguration("npmManager").get<boolean>("autoInstall", true)) {
  ...
  private applyRemoveWithJson(req: MutationRequest, project: WorkspaceProject): void {
    this.output.appendLine(`[json] uninstall ${req.packageId} in ${project.info.name}`);
  ...
  private applyAddWithJson(req: MutationRequest, project: WorkspaceProject, dependencyType: DependencyType, prefix: VersionPrefix): void {
    ...
    this.output.appendLine(`[json] ${req.action} ${req.packageId}@${range} in ${project.info.name}`);
  ...
  private applyPin(req: MutationRequest, project: WorkspaceProject, mode: "pin" | "unpin"): void {
    ...
    this.output.appendLine(`[json] ${mode} ${req.packageId}@${version} in ${project.info.name}`);
