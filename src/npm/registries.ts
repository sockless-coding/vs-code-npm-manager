...
export class RegistryRegistry {
  ...
  refresh(): void {
    ...
    const additional = vscode.workspace
      ...
      .get<{ name: string; url: string }[]>("additionalRegistries", []);
  ...
  async getAuthHeader(requestUrl: string): Promise<string | undefined> {
    ...
    const saved = await this.context.secrets.get(SECRET_PREFIX + registry.name);
  ...
  async promptForCredentials(registryName: string): Promise<boolean> {
    ...
    await this.context.secrets.store(SECRET_PREFIX + registry.name, token);
    return true;
  }

  async clearCredentials(registryName: string): Promise<void> {
    await this.context.secrets.delete(SECRET_PREFIX + registryName);
  }
}
