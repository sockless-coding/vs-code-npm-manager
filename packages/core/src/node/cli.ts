...
export class PackageManagerCli {
  ...
  private exeFor(pm: PackageManagerName): string {
    const override = this.host.getConfig<string>("packageManagerPath", "");
  ...
  run(pm: PackageManagerName, args: string[], cwd?: string, quiet = false): Promise<RunResult> {
    const workingDir = cwd ?? this.host.getWorkspaceRoots()[0] ?? process.cwd();
    const exe = this.exeFor(pm);
    if (!quiet) {
      this.host.log(`> ${pm} ${args.join(" ")}  (${workingDir})`);
    }
    return new Promise((resolve, reject) => {
      execFile(
        ...
        (err, stdout, stderr) => {
          if (!quiet) {
            if (stdout) this.host.log(stdout.replace(/\r?\n$/, ""));
            if (stderr) this.host.log(stderr.replace(/\r?\n$/, ""));
          }
