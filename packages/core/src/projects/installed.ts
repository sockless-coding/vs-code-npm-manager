...
export class InstalledService {
  ...
  private async runEnrichment(token: number): Promise<void> {
    ...
    const includePrerelease = this.host.getConfig<boolean>("defaultIncludePrerelease", false);
  ...
  private usePackageManagerForEnumeration(): boolean {
    return this.host.getConfig<boolean>("usePackageManagerForEnumeration", false);
  }
